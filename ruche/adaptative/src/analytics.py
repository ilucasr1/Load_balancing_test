...
#        client.run_on_scheduler(scheduler_adaptor)
 ...
 #   client.run_on_scheduler(stop_adaptor)
    ...
    memory_samples, global_times = client.run_on_scheduler(get_monitor_results)
    ...
    w_tracker = [0 for i in range(len(global_times))]
    for i, t in enumerate(global_times):
        for memory_lists in memory_samples.values():
            if memory_lists["times"][0] <= t and \
               memory_lists["times"][-1] >= t:
                w_tracker[i] += 1 
    global_times = [datetime.fromtimestamp(ts) for ts in global_times]
    
    plt.plot(global_times, w_tracker, label="Nb workers")
    finalize_plot()
    plt.savefig(filepath + "/plot_nb_workers.png")
    plt.clf()
